...
int main()
{
	...
	while (marineAlive(squad) && zerglingAlive(swarm)) // If anyone is left alive to fight . . .
	{
		...
		{
			for (int i = 0; i < squad.size(); i++) // go through the squad
			{
		...
		{
			for (int i = 0; i < swarm.size(); i++) // go through the squad
			{
				if (marineAlive(squad))
				{
					...
					if (!squad[0].isAlive()) // if the zergling dies, it is removed from the swarm
					{
